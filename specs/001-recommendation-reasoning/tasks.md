...
_No specific foundational tasks for this feature beyond Setup._
...
**Independent Test**: Can create a `Recommendation` object with `PrimaryReason` set to
`RECOMMENDATION_REASON_IDLE` and serialize it.

### Verification (Test First)

- [x] T002 [US1] Create serialization test in `sdk/go/finfocus/v1/recommendation_test.go`
  - Action: Create a new test file (or update existing) to verify that `RecommendationReason`
    can be set, serialized, and deserialized correctly.
...
- [x] T004 [US1] Update `Recommendation` message in `proto/finfocus/v1/costsource.proto`
  - Action: Add `primary_reason` (type: `RecommendationReason`) and `secondary_reasons`
    (type: `repeated RecommendationReason`) fields.
...
- [x] T006 [US2] Verify enum consumption in test/example
  - Action: Extend `recommendation_test.go` to include a test case that switches on `PrimaryReason`
    and asserts the correct string output (mimicking a UI/CLI display logic).
  - File: `sdk/go/finfocus/v1/recommendation_test.go`
