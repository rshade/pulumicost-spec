...
- **Rationale**: Standard Protobuf practice to avoid scoping collisions in generated code (especially C++/Go). Confirmed in spec clarifications.
...
- **Rationale**: Preserves the semantic difference between "I don't know" and "I know it costs nothing".

### SDK Implementation Pattern
- **Question**: How to expose this in the Go SDK?
- **Decision**: Use Functional Options Pattern (e.g., `NewActualCostResponse(..., WithFallbackHint(hint))`).
...
- **Compatibility**: Field number 2 (next available is likely 2, need to check `GetActualCostResponse` definition).
  - `GetActualCostResponse` has `repeated ActualCostResult results = 1;`. Next is 2.

## 3. Best Practices

- **Proto3 Defaults**: The default value (0) must be the "safe" or "backward compatible" behavior. `UNSPECIFIED` mapping to "No Fallback" ensures existing plugins (which send 0 implicitly) don't trigger unwanted fallbacks.
