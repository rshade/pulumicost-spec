...
// azureStandardPrefixLen is the length of "Standard_" prefix in Azure VM sizes.
const azureStandardPrefixLen = 9
...
func extractAzureFamily(vmSize string) string {
	if len(vmSize) <= azureStandardPrefixLen {
		return vmSize
	}
	// Skip "Standard_" prefix
	for i := azureStandardPrefixLen; i < len(vmSize); i++ {
		if vmSize[i] >= '0' && vmSize[i] <= '9' {
			return vmSize[azureStandardPrefixLen:i]
		}
	}
	return vmSize[azureStandardPrefixLen:]
}
