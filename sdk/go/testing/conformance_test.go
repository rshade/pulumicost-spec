...
func addBasicConformanceTests(suite *plugintesting.PluginConformanceSuite) {
	...
	suite.AddTest(plugintesting.ConformanceTest{
		...
		Description: "Plugin must return valid response with correct message types",
	...
	return func(harness *plugintesting.TestHarness) plugintesting.TestResult {
		...
		if resp.GetSummary().GetTotalRecommendations() != 0 {
			return plugintesting.TestResult{
				Method:   "GetRecommendations",
				Success:  false,
				Error:    errors.New("summary should show 0 recommendations for non-matching filter"),
				Duration: duration,
	...
	return func(harness *plugintesting.TestHarness) plugintesting.TestResult {
		...
		if err != nil {
			return plugintesting.TestResult{
				...
				Error:    err,
				Duration: duration,
				Details:  "RPC call with provider filter failed",
			}
		}
		...
		for i, rec := range resp.GetRecommendations() {
			if rec.GetResource() == nil || rec.GetResource().GetProvider() != "aws" {
				return plugintesting.TestResult{
					...
					Duration: duration,
					Details:  "Filter returned non-matching provider",
				}
			}
		}
...
func createGetRecommendationsPaginationNextPageTest() func(*plugintesting.TestHarness) plugintesting.TestResult {
	return func(harness *plugintesting.TestHarness) plugintesting.TestResult {
		...
		// Get first page
		firstResp, err := harness.Client().GetRecommendations(context.Background(), &pbc.GetRecommendationsRequest{
			PageSize: paginationTestPageSize,
		})
		if err != nil {
			return plugintesting.TestResult{
		...
		// Get second page
		secondResp, err := harness.Client().GetRecommendations(context.Background(), &pbc.GetRecommendationsRequest{
			PageSize:  paginationTestPageSize,
...
func createGetRecommendationsRightsizeActionTest() func(*plugintesting.TestHarness) plugintesting.TestResult {
	return func(harness *plugintesting.TestHarness) plugintesting.TestResult {
		...
		// Find a rightsize recommendation and validate its action details
			...
			if rec.GetRightsize() != nil {
				...
				if rightsize.GetCurrentSku() == "" && rightsize.GetCurrentInstanceType() == "" {
					return plugintesting.TestResult{
						Method:   "GetRecommendations",
						Success:  false,
						Error:    errors.New("rightsize action missing current SKU/instance type"),
						Duration: duration,
						Details:  "Rightsize action must have current resource details",
					}
				}
				if rightsize.GetRecommendedSku() == "" && rightsize.GetRecommendedInstanceType() == "" {
					return plugintesting.TestResult{
						Method:   "GetRecommendations",
						Success:  false,
						Error:    errors.New("rightsize action missing recommended SKU/instance type"),
						Duration: duration,
						Details:  "Rightsize action must have recommended resource details",
					}
				}
				return plugintesting.TestResult{
					Method:   "GetRecommendations",
					...
					Duration: duration,
					Details: fmt.Sprintf(
						"Rightsize action valid: %s -> %s",
						rightsize.GetCurrentSku(),
						rightsize.GetRecommendedSku(),
					),
				}
			}
		}

		// No rightsize recommendations found - that's okay, test passes
		return plugintesting.TestResult{
			Method:   "GetRecommendations",
			Success:  true,
			Duration: duration,
			Details:  "No rightsize recommendations available (filter returned 0 results)",
		}
	}
}
