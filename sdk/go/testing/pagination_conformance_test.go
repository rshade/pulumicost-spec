...
func TestPaginationConformance_FirstPage(t *testing.T) {
	...
	plugin.ActualCostDataPoints = 200
...
func TestPaginationConformance_LastPage(t *testing.T) {
	plugin := plugintesting.NewMockPlugin()
	plugin.ActualCostDataPoints = 120
	harness := plugintesting.NewTestHarness(plugin)
	harness.Start(t)
	defer harness.Stop()
...
func TestPaginationConformance_EmptyResult(t *testing.T) {
	plugin := plugintesting.NewMockPlugin()
	plugin.ActualCostDataPoints = 50
	harness := plugintesting.NewTestHarness(plugin)
	harness.Start(t)
	defer harness.Stop()
...
func TestPaginationConformance_OversizedPage(t *testing.T) {
	plugin := plugintesting.NewMockPlugin()
	plugin.ActualCostDataPoints = 500
	harness := plugintesting.NewTestHarness(plugin)
	harness.Start(t)
	defer harness.Stop()
...
func TestPaginationConformance_BackwardCompat(t *testing.T) {
	plugin := plugintesting.NewMockPlugin()
	plugin.ActualCostDataPoints = 24 // Small dataset like legacy plugins
	harness := plugintesting.NewTestHarness(plugin)
	harness.Start(t)
	defer harness.Stop()
