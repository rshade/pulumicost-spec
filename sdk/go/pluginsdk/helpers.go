...
// Returns recommendations that match ALL specified filter criteria.
...
// Returns the page of recommendations and the next page token (empty if last page).
...
func CalculateRecommendationSummary(
	...
	for _, rec := range recommendations {
		...
		if impact := rec.GetImpact(); impact != nil {
			...
			if impact.GetCurrency() != "" {
				detectedCurrency = impact.GetCurrency()
			}
		}
	}
