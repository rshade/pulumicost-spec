...
func TestPaginateActualCosts_NegativePageSize(t *testing.T) {
	...
	// Negative page_size with a page_token should normalize to default page size
	page, nextToken, totalCount, err := pluginsdk.PaginateActualCosts(results, -1, pluginsdk.EncodePageToken(0))
	if err != nil {
		t.Fatalf("unexpected error for negative page_size with page_token: %v", err)
	}
	if len(page) != pluginsdk.DefaultPageSize {
		t.Errorf("expected %d results (default page size), got %d", pluginsdk.DefaultPageSize, len(page))
	}
	if nextToken == "" {
		t.Error("expected next page token for paginated results")
	}
	if totalCount != 100 {
		t.Errorf("expected total_count=100, got %d", totalCount)
	}

	// Negative page_size without page_token should normalize and return all results (legacy path)
	...
	if len(page) != 100 {
		t.Errorf("expected 100 results in legacy mode, got %d", len(page))
	}
	if totalCount != 100 {
		t.Errorf("expected total_count=100, got %d", totalCount)
	}
