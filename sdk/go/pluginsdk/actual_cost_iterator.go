...
func (it *ActualCostIterator) Next() bool {
	...
	// If this page is empty but there's a next token, fetch the next page
	// This handles cases where a page has no results due to filtering or data deletion
	if len(it.current) == 0 {
		if it.pageToken != "" {
